...
```mermaid
flowchart LR
    U[Users\nCustomer / Admin / Super Admin] --> F[Frontend\nReact + Redux]
    F -->|JWT-authenticated REST calls| B[Backend API\nSpring Boot]

    B --> S[Spring Security\nJWT + Role Authorization]
    B --> BL[Business Layer\nOrders, Cart, Menu, Reviews, Notifications]
    BL --> D[(MySQL Database)]

    BL --> P[Stripe Payment Gateway]
    BL --> M[Email Service\nSpring Mail]
```

This architecture separates presentation, security, and domain logic to keep the system modular and scalable while supporting role-based experiences.
